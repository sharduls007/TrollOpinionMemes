...
def get_args(parser):
    ...
    parser.add_argument("--task", type=str, default="TrollOpinionMeme", choices=["TrollOpinionMeme", "vsnli", "food101"])
...
def model_predict(i_epoch, data, model, args, criterion):
    with torch.no_grad():
        tgts = []
        preds = []
        outs = []
        for batch in data:
            _, out, tgt = model_forward(i_epoch, model, args, criterion, batch)
            if args.task_type == "multilabel":                
                pred = torch.sigmoid(out).cpu().detach().numpy() > 0.5
            else:
                prob = torch.nn.functional.softmax(out, dim=1).cpu().detach().numpy()
                pred = torch.nn.functional.softmax(out, dim=1).argmax(dim=1).cpu().detach().numpy()
            outs.append(list(prob))
            preds.append(pred)
            tgts.append(tgt)

    if args.task_type == "multilabel":
        preds = np.vstack(preds)
        with open(os.path.join(args.savedir, "test_labels_pred" + str(i_epoch) + ".txt"), "w") as fw:
            fw.write(
                "\n".join([" ".join(["1" if x else "0" for x in p]) for p in preds])
            )
    else:
        outs = [l for sl in outs for l in sl]
        tgts = [l for sl in tgts for l in sl]
        preds = [l for sl in preds for l in sl]
        dict_output = {'proba': outs, 'label': preds}
        df_output = pd.DataFrame(dict_output)
        df_output.to_csv('output.csv', sep=',', index=None)
#         with open(os.path.join(args.savedir, "test_labels_pred" + str(i_epoch) + ".txt"), "w") as fw:
#             fw.write("\n".join([str(x) for x in preds]))
#         with open(os.path.join(args.savedir, "prob.txt"), "w") as fw:
